...
function getMov(event) {
  ...
  fetch(
    "https://ott-details.p.rapidapi.com/advancedsearch?start_year=" +
      startYearInput +
      "&end_year=" +
      endYearInput +
      "&min_imdb=" +
...
function getIng(event) {
  ...
  fetch(
    "https://tasty.p.rapidapi.com/recipes/list?from=0&size=20&cuisine=" +
      cusInput +
      "&q=" +
      ingInput,
    options
    ...
    //   this assigns a HTML class to target in Css
      thumbDisplay.setAttribute("class", "food-image");
      ...
      if (data.results[fselection].instructions) {
        ...
        while (data.results[fselection].instructions[i]) {
          ...
          screen.textContent =
            data.results[fselection].instructions[i].display_text;
